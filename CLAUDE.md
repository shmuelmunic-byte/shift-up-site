...
### 📊 התקדמות בנייה (עודכן 2026-06-24, סשן 2)
- ✅ **סקשן 1 — Hero** (`Hero.jsx`): נבנה מחדש. כותרת נעולה, 2CTA (טופס ראשי/וואטסאפ משני), עוגן מחיר 1,500₪, scarcity, trust chips, ציאן ב-aurora.
- ✅ **סקשן 2 — רצועת אמון** (`TrustStrip.jsx`): חדש. סלקטיביות (2–3 ברבעון) + זמינות חיה + שיחת היכרות חינם.
- ✅ **סקשן 3 — כאב** (`PainSection.jsx`): חדש. הוק "נמאס לרדוף..." + 4 כרטיסים + מעבר הקלה ירוק.
- ✅ **סקשן 4 — 3 שלבים** (`Process.jsx`): התאמת תוכן fallback (פיצוח/שיפט/אקשן). **טעון סנכרון DB ↓**
- ⬜ **הבא: סקשן 5 — "מה תקבל"** ("מה יוצא לי?" → לידים + שקט), ואז 6 (למה אני לא סוכנות), 7 (הוכחה+פורטפוליו), טופס לידים (10).
- שינוי מבנה: `MarqueeSection` + `Stats` **נותקו מ-App** (קיימים על הדיסק). הסדר ב-`App.jsx`: Hero → TrustStrip → Pain → Process → WhyMe → Manifesto → About → FAQ → ...
