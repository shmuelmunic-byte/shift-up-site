...
function EnFooter() {
  ...
  const copyEmail = (e) => {
    e.preventDefault();
    navigator.clipboard.writeText('shmuelmunic@gmail.com').then(() => {
      setEmailCopied(true);
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {socials.map(s => {
            ...
            const active  = isEmail && emailCopied;
            return (
              <div key={s.label} style={{ position: 'relative' }}>
                <a href={s.href} aria-label={active ? 'Copied!' : s.label}
                  target={s.target} rel={s.target === '_blank' ? 'noopener noreferrer' : undefined}
                  onClick={isEmail ? copyEmail : undefined}
                  style={{ width: 40, height: 40, borderRadius: '50%', background: active ? 'oklch(0.78 0.20 145 / 0.2)' : 'var(--surface-1)', border: `1px solid ${active ? 'oklch(0.78 0.20 145 / 0.5)' : 'var(--surface-2)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: active ? 'var(--brand-prime)' : 'var(--text-muted)', textDecoration: 'none', transition: 'background 0.3s, color 0.3s, border-color 0.3s' }}
                  onMouseEnter={e => { if (active) return; e.currentTarget.style.background='oklch(0.78 0.20 145 / 0.15)'; e.currentTarget.style.color='var(--brand-prime)'; }}
                  onMouseLeave={e => { if (active) return; e.currentTarget.style.background='var(--surface-1)'; e.currentTarget.style.color='var(--text-muted)'; }}
                >
                  {active ? (
                ...
                {active && (
                  <div style={{ position: 'absolute', bottom: '110%', left: '50%', transform: 'translateX(-50%)', background: 'var(--surface-2)', color: 'var(--brand-prime)', fontSize: '0.7rem', fontWeight: 700, padding: '4px 10px', borderRadius: 6, whiteSpace: 'nowrap', pointerEvents: 'none' }}>
                    Copied ✓
                  </div>
                )}
              </div>
